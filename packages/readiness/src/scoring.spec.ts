...
describe('Readiness Engine scoring', () => {
  it('is deterministic for identical signals', () => {
    fc.assert(fc.property(fc.array(signalArbitrary), (signals) => {
      expect(computePillarScore('provision', signals)).toBe(computePillarScore('provision', signals));
    }));
  });

  it('computes the weighted average of pillar scores', () => {
    expect(computeOverallReadiness({ protection: 80, provision: 60, preparation: 100, prosperity: 80 }, {
      protection: 1, provision: 2, preparation: 1, prosperity: 0,
    })).toBe(75);
  });
