...
interface Account { id: string; name: string; type: string; currentBalance: string; }
...
interface CashFlowProjection { date: string; balance: string; credits: string; debits: string; }
interface CashFlowResult { projections: CashFlowProjection[]; belowZeroNotifications: { accountId: string; accountName: string; date: string; projectedAmount: string; }[]; }
...
export default function RecurringPage() {
  ...
  const accountsQuery = useQuery({ queryKey: ['accounts'], queryFn: () => apiClient.get<Account[]>('/accounts') });
  ...
  const cashFlowQuery = useQuery({ queryKey: ['cashflow', accountId], queryFn: () => apiClient.get<CashFlowResult>(`/cashflow/forecast?accountId=${accountId}`), enabled: !!accountId });
  ...
  return (
    <div className="space-y-6">
      ...
      {cashFlowQuery.data && cashFlowQuery.data.projections.length > 0 && (() => {
        ...
        const lowest = projections.reduce((min, p) => Number(p.balance) < Number(min.balance) ? p : min, projections[0]);
        return (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="card"><span className="card-title">END BALANCE (90D)</span><p className="text-2xl font-bold tabular-nums text-content-primary">${formatCurrency(endBalance)}</p></div>
            <div className="card"><span className="card-title">LOWEST BALANCE</span><p className="text-2xl font-bold tabular-nums text-accent-red">${formatCurrency(Number(lowest.balance))}</p></div>
            <div className="card"><span className="card-title">LOWEST DATE</span><p className="text-2xl font-bold text-content-primary">{new Date(lowest.date).toLocaleDateString()}</p></div>
          </div>
        );
      })()}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="w-56">
            <select value={accountId} onChange={(e) => setAccountId(e.target.value)} className="input">
              ...
              {accountsQuery.data && accountsQuery.data.length === 0 && <option value="">No accounts</option>}
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-edge">
                  {cashFlowQuery.data.projections.slice(0, 30).map((p, i) => (
                    <tr key={i} className="hover:bg-surface-elevated transition-colors"><td className="px-6 py-3 text-content-secondary">{new Date(p.date).toLocaleDateString()}</td><td className="px-6 py-3 text-right tabular-nums text-accent-green">${formatCurrency(Number(p.credits))}</td><td className="px-6 py-3 text-right tabular-nums text-accent-red">${formatCurrency(Number(p.debits))}</td><td className="px-6 py-3 text-right tabular-nums text-content-primary font-medium">${formatCurrency(Number(p.balance))}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
