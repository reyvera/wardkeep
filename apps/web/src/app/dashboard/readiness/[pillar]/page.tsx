...
import { AlertTriangle, ArrowLeft, CheckCircle2, CircleHelp, Shield, TrendingUp, Wallet, Hammer, PiggyBank } from 'lucide-react';
...
function signalColor(type: Signal['type']) {
  return type === 'risk' ? 'var(--accent-red)' : type === 'warning' ? 'var(--accent-amber)' : 'var(--accent-green)';
}
...
export default function ReadinessPillarPage() {
  ...
  if (!meta) {
    return <div className="card text-center py-12"><p className="text-content-secondary">This readiness area does not exist.</p><Link href="/dashboard" className="btn-secondary mt-4">Back to dashboard</Link></div>;
  }

  if (readinessQuery.isLoading) {
    return <div className="space-y-6"><div className="skeleton h-8 w-40" /><div className="card"><div className="skeleton h-48 w-full" /></div></div>;
  }

  if (readinessQuery.isError) {
    return <div className="card text-center py-12"><p className="text-content-secondary">Unable to load this readiness area.</p><Link href="/dashboard" className="btn-secondary mt-4">Back to dashboard</Link></div>;
  }
  ...
  return (
    <div className="space-y-6">
      <Link href="/dashboard" className="btn-ghost text-xs w-fit"><ArrowLeft size={14} />Dashboard</Link>
      ...
      {coverage === 0 ? (
        ...
        <section className="card">
          ...
          {signals.length === 0 ? <p className="text-sm text-content-tertiary">No current factors were returned for this area.</p> : <ul className="space-y-4">{signals.map((signal, index) => <li key={`${signal.capabilityId}-${index}`} className="flex gap-3"><AlertTriangle size={17} className="mt-0.5 flex-shrink-0" style={{ color: signalColor(signal.type) }} /><div><p className="text-sm text-content-primary">{signal.summary}</p><p className="text-xs text-content-tertiary mt-1">{signal.capabilityId.replace(/-/g, ' ')} · current calculation</p></div></li>)}</ul>}
        </section>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="card">
          ...
          {meta.observed.length === 0 ? <p className="text-sm text-content-tertiary">No direct factors are evaluated in this pillar yet.</p> : <ul className="space-y-3">{meta.observed.map((factor) => <li key={factor.capability} className="flex gap-2 text-sm"><CheckCircle2 size={16} className={observedCapabilities.has(factor.capability) ? 'text-accent-green' : 'text-content-tertiary'} /><span className="text-content-primary">{factor.label}</span></li>)}</ul>}
        </section>
        <section className="card">
