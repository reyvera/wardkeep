...
export default function ReadinessPage() {
  ...
  if (readinessQuery.isLoading) {
    return (
      <div>
        <h1 className="text-page-title mb-6">Household Readiness</h1>
  ...
  return (
    <div>
      ...
      {/* All Signals */}
      <div className="card">
        <h3 className="card-title">All Signals ({data.signals.length})</h3>
        {data.signals.length === 0 ? (
          <p className="text-sm text-content-tertiary">
            No signals generated. Set up accounts, budgets, and recurring transactions to enable readiness tracking.
          </p>
        ) : (
          <ul className="space-y-2 max-h-96 overflow-y-auto">
            {data.signals
              ...
              .map((signal, i) => {
                const Icon = getSignalIcon(signal.type);
                const color = getSignalColor(signal.type);
                const pillarMeta = PILLAR_META[signal.pillar];
                return (
                  <li key={i} className="flex items-start gap-3 py-2 border-b border-edge last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-content-tertiary mt-0.5">
                        {pillarMeta?.label ?? signal.pillar} &middot; {signal.capabilityId}
                      </p>
                    </div>
                    <span
                      ...
                      style={{ color }}
                    >
                      {signal.magnitude > 0 ? '+' : ''}{signal.magnitude}
                    </span>
                  </li>
                );
              })}
