...
export default function AdvisorMemoryPage() {
  ...
  const create = useMutation({
    mutationFn: () => apiClient.post('/advisor/memory', { kind, summary, ...(expiresAt ? { expiresAt: new Date(`${expiresAt}T00:00:00.000Z`).toISOString() } : {}) }),
    onSuccess: () => { setSummary(''); setExpiresAt(''); queryClient.invalidateQueries({ queryKey: ['advisor-memory'] }); },
  });
