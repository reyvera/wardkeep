...
/** Calls the API's trusted endpoint to create every backup that is due. */
export async function processScheduledBackups(): Promise<void> {
  const encryptionKey = process.env['ENCRYPTION_KEY'];
  if (!encryptionKey) {
    throw new Error('ENCRYPTION_KEY is required to run scheduled backup jobs');
  }
  ...
  const response = await fetch(`${apiUrl}/internal/backups/run-due`, {
    method: 'POST',
    headers: { 'x-wardkeep-worker-token': workerToken },
  });

  if (!response.ok) {
    throw new Error(`Scheduled backup request failed with HTTP ${response.status}`);
  }
}
