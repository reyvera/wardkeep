...
export function emergencyPreparednessSignal(items: Array<{ isComplete: boolean; category: string }>): Signal | null {
  if (items.length === 0) return null;
  const incomplete = items.filter((item) => !item.isComplete);
  if (incomplete.length === 0) return { capabilityId: 'emergency-preparedness', type: 'positive', magnitude: 1, pillar: 'protection', weight: 0.5, summary: `${items.length} preparedness items are recorded complete. This is a household checklist record, not a safety certification.` };
