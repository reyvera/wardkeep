import { Decimal } from 'decimal.js'; import { describe, expect, it } from 'vitest'; import { homeAssetLifespanSignal, leaseEndingSignal, plannedExpenseSignal, vehicleMaintenanceSignal } from './preparation.generator';
...
describe('vehicle preparation signals', () => {
  ...
  it('flags only a home asset at its recorded expected lifespan', () => {
    expect(homeAssetLifespanSignal({ name: 'Main HVAC', installedAt: new Date('2010-06-01'), expectedLifespanYears: 15, replacementCost: new Decimal(8500) }, now)).toMatchObject({ capabilityId: 'home-assets', type: 'risk', magnitude: -4 });
  });
});
