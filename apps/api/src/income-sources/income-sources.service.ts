...
type IncomeSourceInput = {
  ...
  employmentPayType?: 'SALARY' | 'HOURLY' | 'OTHER' | null;
...
export class IncomeSourcesService {
  ...
  create(userId: string, dto: IncomeSourceInput & { name: string; frequency: NonNullable<IncomeSourceInput['frequency']> }) { return this.prisma.incomeSource.create({ data: { ...dto, userId, expectedNetAmount: dto.expectedNetAmount ? new Decimal(dto.expectedNetAmount) : null, hourlyRate: dto.hourlyRate ? new Decimal(dto.hourlyRate) : null, typicalHoursPerPayPeriod: dto.typicalHoursPerPayPeriod ? new Decimal(dto.typicalHoursPerPayPeriod) : null, nextExpectedDate: dto.nextExpectedDate ? new Date(`${dto.nextExpectedDate}T00:00:00.000Z`) : null, reviewDate: dto.reviewDate ? new Date(`${dto.reviewDate}T00:00:00.000Z`) : null } }); }
  async update(userId: string, id: string, dto: IncomeSourceInput) { const existing = await this.prisma.incomeSource.findFirst({ where: { id, userId }, select: { id: true } }); if (!existing) throw new NotFoundException('Income source not found'); return this.prisma.incomeSource.update({ where: { id }, data: { ...dto, ...(dto.expectedNetAmount !== undefined && { expectedNetAmount: dto.expectedNetAmount ? new Decimal(dto.expectedNetAmount) : null }), ...(dto.hourlyRate !== undefined && { hourlyRate: dto.hourlyRate ? new Decimal(dto.hourlyRate) : null }), ...(dto.typicalHoursPerPayPeriod !== undefined && { typicalHoursPerPayPeriod: dto.typicalHoursPerPayPeriod ? new Decimal(dto.typicalHoursPerPayPeriod) : null }), ...(dto.nextExpectedDate !== undefined && { nextExpectedDate: dto.nextExpectedDate ? new Date(`${dto.nextExpectedDate}T00:00:00.000Z`) : null }), ...(dto.reviewDate !== undefined && { reviewDate: dto.reviewDate ? new Date(`${dto.reviewDate}T00:00:00.000Z`) : null }) } }); }
