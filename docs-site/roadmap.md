...
| Trust and coverage     | In progress | Explicit known/partial/not-evaluated states, factor-level provenance and evidence states, truthful manual-versus-synchronized freshness summaries, hardened burn-rate calculations, and actual-spend budget pacing are live; source-specific freshness rules remain                                           |
...
| Scenarios and planning | Partial     | A Dashboard comparison can vary or remove one already-evaluated readiness factor, recompute the direct pillars and derived Peace, and show the before/after evidence boundaries. It is temporary, deterministic, and deliberately not a prediction, plan, recommendation, or record change.                   |
| Household transition   | Partial     | A privacy-first survivor walkthrough, same-household Wardkeep-only lead transition, owner-approved immutable handoff summary, trusted-access consent, revocation, lockout, and audit trail are live. They organize recorded information without determining legal authority or automatically granting access. |
