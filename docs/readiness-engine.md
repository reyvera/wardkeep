...
- Daily snapshots preserve overall and pillar score history as well as the contributing signals. The readiness response includes up to 90 available daily snapshots plus 7-, 30-, and 90-day comparison windows when an older snapshot exists. The Dashboard readiness card provides a visible 7d / 30d / 90d selector; every label uses the actual elapsed days represented and only shows an overall trend for a sufficiently complete assessment.
