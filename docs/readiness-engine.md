...
> **Strategic taxonomy note:** Wardkeep's active product framework has Protection, Provision, Prosperity, and derived Peace. Model 1's `Preparation` pillar is retained only in explicitly labeled legacy history. This document describes the deployed model-2 implementation; [product-differentiation.md](product-differentiation.md) defines the product rationale.

The transition was not a UI rename. Model 2 uses versioned score history, reclassifies each former Preparation signal by household consequence, publishes revised score/coverage changes, and serves the API and dashboard contract atomically. Snapshots are uniquely stored by household, day, and model version, so model 2 cannot overwrite legacy model-1 data. The dashboard presents a legacy-history selector only when more than one series exists, and never treats cross-model values as one trend. Historical Preparation values remain identified as legacy data unless recalculated from the same source observations under the new deterministic rules.
...
| Provision   | Budget pace, cash-flow forecast with recorded one-time future events, bill coverage                                                | Partial        | Add income stability, essential obligations, and recurring-payment reliability.                                                   |
...
| Peace       | Derived from the least-ready observed direct pillar, recent volatility, and recorded administrative attention                     | Derived        | Improve its explanation and make unknown upstream coverage visible.                                                               |
...
- The observed overall score re-normalizes the published model-2 direct weights across evaluated pillars: Protection 35%, Provision 35%, Prosperity 30%. It is `null` when no direct pillar is evaluated and is explicitly marked partial until sufficient coverage exists. Peace is derived and is not weighted into overall readiness.
...
### Future household-transition support: organization, not authority

The Household Transition workspace supports two distinct workflows: an incapacity continuity plan for a living household member, and an after-death settlement plan for survivors. It helps a surviving spouse or another explicitly invited trusted person work through recorded accounts, policies, document locations, recurring obligations, and user-entered arrangement checklists. It may include optional funeral or memorial planning checklists and prompts to contact appropriate professionals. Wardkeep does not determine whether someone is incapacitated or deceased, validate an executor, personal representative, or power of attorney, bypass authentication, grant access from a recorded designation, or provide legal, tax, medical, financial, or funeral-direction advice. Any trusted-person access is separately invited, approved, scoped, revocable, and auditable. The workflow stores locations and notes by default—not sensitive document contents—and death or access status never itself changes a readiness score. It never automatically releases information due to inactivity or a reported death.
...
## Provision: recorded future obligations

Provision remains incomplete until the household records the relevant future obligations. An active saved one-time future bill or deposit is assigned to an account and included in that account's deterministic 90-day cash-flow projection; when resolved, it remains as household history but no longer affects future projections. A recorded planned expense with a due date in the next 30 days is separately compared with only the funds the household explicitly marks as set aside. Wardkeep does not infer affordability, verify that unrecorded funds exist, or evaluate future costs that have not been recorded.
...
The resettable `demo@wardkeep.app` fixture includes 90 days of clearly labeled demo readiness history plus three confirmed and one detected recurring-bill record, so the Dashboard trends and Cash Flow & Recurring workflow can be verified immediately. Its generated amounts, merchants, and account choices are deterministic for repeatable verification and screenshots; dates remain relative to the day it is seeded. The first live readiness check replaces today's demo point with the current calculated score.
