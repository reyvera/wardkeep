...
Provision remains incomplete until the household records the relevant future obligations. A saved one-time future bill or deposit is assigned to an account and included in that account's deterministic 90-day cash-flow projection; a recorded planned expense with a due date in the next 30 days is separately compared with only the funds the household explicitly marks as set aside. Wardkeep does not infer affordability, verify that unrecorded funds exist, or evaluate future costs that have not been recorded.
