...
- **Wardkeep recommends**, a durable, prioritized list of active risk and warning actions that connects an observation to the relevant household workflow (such as accounts, policies, budget, cash flow, recurring bills, debt, or an explainable readiness factor). The Dashboard and Recommendations workspace refresh readiness before showing actions. People can complete or dismiss a recommendation, and review completed, dismissed, or resolved action history; financial impact previews are still forthcoming;
